...
void Hunter() {
	...
	#pragma omp barrier
	double tmp= rand();
	if(NowNumDeer !=0 && NowNumDeer !=1) {
		
		KilledDeer=((int)tmp % NowNumDeer)+1;
		if(KilledDeer == NowNumDeer) {
			KilledDeer--;
		}
	} else {
...
void Watcher() {
	...
	if( NowPrecip < 0. )
		NowPrecip = 0.;
	printf("Grain Growth= %f ,DeerGrowth= %d ,Temp= %f ,Precipitation= %f ,KilledDeer=%d\n",NowHeight,NowNumDeer,NowTemp, NowPrecip,KilledDeer);
	#pragma omp barrier
}
